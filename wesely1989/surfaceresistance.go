...
func r_dc(G, Θ float64) float64 {
	return 100 * (1. + 1000.*1./(G+10.)) / (1. + 1000.*Θ)
}
...
// (leaf cuticular resistance in healthy vegitation)
...
func r_lux(Hstar, fo float64, iSeason, iLandUse int,
	rain, dew, isSO2, isO3 bool) (rlux float64) {
	if dew {
		if isSO2 {
			if iLandUse == 0 {
			...
			rlux = 1. / (1./(1./(3*r_lu[iSeason][iLandUse])) + 1.e-7*Hstar +
				fo/rluO) // equation 14
		}
	} else if rain {
		if isSO2 {
...
func r_clx(Hstar, fo float64, iSeason, iLandUse int) float64 {
	return 1. / (Hstar/(1.e-5*r_clS[iSeason][iLandUse]) +
...
func r_gsx(Hstar, fo float64, iSeason, iLandUse int) float64 {
	return 1. / (Hstar/(1.e-5*r_gsS[iSeason][iLandUse]) +
